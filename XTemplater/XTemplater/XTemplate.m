...
+ (instancetype) templateNamed:(NSString*)name
	...
	t.name = name; t.iconPath = iconPath; t.platform = platform; t.category = category; return t;
}
...
- (void)setCategoryFrom:(id)sender
{
	[self setCategory:[[sender selectedItem] tag]];
	_customCategory.hidden = _category != CUSTOM;
}
- (void)setPlatformFrom:(id)sender
{
    [self setPlatform:[sender selectedSegment]];
